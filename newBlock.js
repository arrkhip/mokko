...
function sassConnect(blockName) {
  return new Promise((resolve, reject) => {
    fs.appendFile(`app/stylus/style.styl`, `\n@import '../blocks/${blockName}/${blockName}.styl'`, err => {
      if (err) {
        reject(`ERR>>> Failed to rewrite style.styl`);
      } else {
        resolve();
      }
    });
  ...
  const makeBlock = blockName => {
    ...
    return validateBlockName(blockName)
      ...
      .then(files => {
        ...
        console.log(line);
        console.log(`The block has just been created in 'blocks/${blockName}'`);
        console.log(line);
